...
In this chapter, we present a step-by-step guide for performing MD simulations using GROMACS. The workflow begins with a protein solvated in water and introduces the essential stages of a typical MD simulation pipeline.  Once the basic workflow is established, the same methodology can be extended to more complex biological systems, including:
...
## 15.2 Standard Procedure of Molecular Dynamcis Simulation
A typical MD simulation involves a series of rigorous steps to ensure the physical validity and stability of the system. Figure 1 and Below is the standard workflow, focusing on the GROMACS pipeline.

<figure>
	<img src="/assets/img/Biophysics/Chapter-15/C15_1.png" alt="Description of the image" style= "width: 100%;"> 
	<figcaption>Standard Procedure of MD simulation with GROMACS</figcaption>
</figure>

1. **Structure Acquisition and Repair**
The first step is obtaining the molecular structure, usually from the Protein Data Bank (PDB). Raw experimental structures often contain "artifacts" that must be fixed before simulation.
* **Structure Cleaning**: Before starting the simulation, the protein structure should be checked and corrected for potential issues such as missing atoms, incomplete residues, or broken bonds. These problems are common in experimentally determined structures and may cause errors during topology generation.
**Why Structure Fixing is Important**: Missing or incorrect information in a PDB file will cause the topology generation (via `pdb2gmx`) to fail. Common issues include:
- Missing Atoms: Incomplete backbone or side-chain atoms lead to undefined force field parameters.
- Unresolved Residues: Flexible loops often lack coordinates in XRD structures. These must be modeled (e.g., using Modeller or AlphaFold) to avoid gaps.
- Chain Termination: Each chain must end with a `TER` record or proper capping to define boundaries.

2. **System Preparation & Topology Generation**
Once the structure is clean, you must map the physical atoms to mathematical parameters defined by a Force Field. 
* ` gmx pdb2gmx` will generates three key files
    - **Topology** (`.top`):   The topology file serves as a blueprint for the simulation. It defines atom types, charges, and bonded parameters (bonds, angles, dihedrals). `.top` file tells GROMACS how those atoms interact based on the chosen force field.
    - **Position restraint** (`posre.itp`): Restricts heavy atoms (C, N, O) to their starting positions, preventing the protein from collapsing or unfolding during early equilibration.
    - **Processed Structure** (`.gro`): The GROMACS structure file contains atomic positions and velocities. while GROMACS supports `.pdb`, the `.gro` format is the native standard for defining the simulation box and velocities. 

3. **Solvation and Ionization**: Placing the molecule in a "box" and filling it with solvent (typically TIP3P water) and add counter-ions (like $$Na^+$$ or $$Cl^-$$) to neutralize the system's net charge and achieve a specific ionic strength (e.g, 0.15 M Nacl) to mimic physiological conditions.

4. **Equilibration**: A system cannot jump straight into a high-speed simulation. It must be "relaxed" in stages to prevent the simulation from "blowing up" due to high-energy overlaps.
    - **Energy Minimization**: Uses algorithms like Steepest Descent to remove steric clashes and find a local potential energy minimum.
    - **NVT Ensemble** (Constant Number, Volume, and Temperature). Also called "Isothermal-isochoric." The system is heated or cold to the target temperature (e.g., 300 K) while keeping the volume fixed.
    iii. **NPT Ensemble** (Constant Number, Pressure, and Temperature): Also called "Isothermal-isobaric." This stabilizes the system's pressure (usually 1.0 bar) and adjusts the density to realistic levels.

5. **Production MD**: Once the temperature and pressure are stable, the position restraints are removed.
    - Execution: The simulation runs for the desired timescale (nanoseconds to microseconds).
    - Trajectory: The output is a `.xtc` or `.trr` file, which is essentially a "movie" of the atomic positions over time.

6. **Analysis**: The final step is extracting meaningful biological data from the raw trajectories.
    - **Structural Stability**: Calculating RMSD (Root Mean Square Deviation) to see if the protein stayed folded.
    - **Flexibility**: Calculating RMSF (Root Mean Square Fluctuation) to identify mobile regions.
    - **Interactions**: Monitoring Hydrogen bonds, Salt bridges, or the Radius of Gyration ($$R_g$$) to assess compactness.

## 15.3 MD Simulation of Single Protein
...
1. **Download the Structure**: Download the protein structure (pdf Legacy) from the  <a href="https://www.rcsb.org/" target="_blank"> RCSB  Protein Data Bank</a>. 
2. **Fixing the Structure**:
Always check your **.pdb** file for entries listed under the comment MISSING, as these entries indicate either atoms or whole residues that are not present in the crystal structure.  For this step, we will use the online platform: <a href="https://mmb.irbbarcelona.org/biobb-wfs/structure/step1/setup" target="_blank"> BioExcel Buidling Blocks Workflows (BioBB-WFS) </a> 
<br> 
or we will use MODELLER through ChrimeraX. <br> These tools allow users to inspect and repair structural problems in PDB files before running molecular dynamics simulations. <br> After fixing the structure, the save the corrected PDB file as `1AKI_clean.pdb`.

3. **Removing Crystal Water**:
Typically, we need to be removed the crytal water
**Method 1**: Removing the the crystal waters using ChimeraX using command prompt  
* `del solvent` 
* `save path/1AKI_clean.pdb #1`
* Go to File -> Save As
* Choose: Format PDB
> Note:  If you want save only specific region of protein, 
* `select protein`
* `save path/1AKI_clean.pdb selectedOnly true`
{: .prompt-info}

**Method 2**: Remove the water using the bash shell
* `grep -v HOH 1AKI_clean.pdb > 1AKI_clean.pdb` - type this in the command line.

> Note: 
Avoid using Microsoft word processing software for removing the water or endit the pdb file, as it may alter the file format
{: .prompt-warning}
...
4. **Generate Molecular Topology Using pdb2gmx**:  
> 
...
* `gmx pdb2gmx -f 1AKI_clean.pdb -o 1AKI_processed.gro -water spce`- Upon execution of this commands, GROMACS will prompt you to choose a force field. Select: `15: OPLS-AA/L all-atom force field (2001 aminoacid dihedrals)` which is a standard force field in gromacs. 
...
<figure class="half-width">
	<img src="/assets/img/Biophysics/Chapter-15/C15_3.png" alt="Description of the image" style= "width: 100%;"> 
	<figcaption>Typical output of position restrain file"</figcaption>
</figure>

5. **Common Water Models in GROMACS**
...
4. **Create Box and Solvate**: We will follow two steps
There are two steps:
...
8. **MD Final Production Run**
To run using the batch job, 
9. **Analysis**
*:wq
